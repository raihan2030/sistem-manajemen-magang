...
<body class="bg-[#f3f4f6] text-[#1f2937] antialiased flex h-screen overflow-hidden">
    ...
    <div id="sidebarOverlay" onclick="toggleSidebar()" class="fixed inset-0 bg-gray-900/50 z-40 hidden transition-opacity lg:hidden backdrop-blur-sm"></div>

    <!-- SIDEBAR ADMIN -->
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-[#F8F9FF] border-r border-gray-200 flex flex-col justify-between h-full transform -translate-x-full transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0">
        <div>
            <!-- Logo & Title Horizontal -->
            <div class="flex items-center justify-between p-6 border-b border-gray-100">
                <div class="flex items-center">
                    <img src="{{ asset('images/logo-bjm.jpg') }}" alt="Logo Banjarmasin" class="w-10 h-10 object-contain mr-3">
                    <div>
                ...
                <button onclick="toggleSidebar()" class="lg:hidden text-gray-400 hover:text-red-500 focus:outline-none transition">
            ...
            <nav class="px-4 pt-6 space-y-1.5 overflow-y-auto">
                ...
                class="flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->routeIs('admin.permohonan') ? 'bg-[#00236F] text-white shadow-sm' : 'text-[#1f2937]/70 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 mr-3.5 flex-shrink-0 {{ request()->routeIs('admin.permohonan') ? 'text-white' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                class="flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->routeIs('admin.kapasitas') ? 'bg-[#00236F] text-white shadow-sm' : 'text-[#1f2937]/70 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 mr-3.5 flex-shrink-0 {{ request()->routeIs('admin.kapasitas') ? 'text-white' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                class="flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->routeIs('admin.upload_sertifikat') ? 'bg-[#00236F] text-white shadow-sm' : 'text-[#1f2937]/70 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 mr-3.5 flex-shrink-0 {{ request()->routeIs('admin.upload_sertifikat') ? 'text-white' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                class="flex items-center px-4 py-3 rounded-xl text-sm font-semibold transition {{ request()->routeIs('admin.notifikasi') ? 'bg-[#00236F] text-white shadow-sm' : 'text-[#1f2937]/70 hover:bg-gray-100' }}">
                    <svg class="w-5 h-5 mr-3.5 flex-shrink-0 {{ request()->routeIs('admin.notifikasi') ? 'text-white' : 'text-gray-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="p-4 border-t border-gray-200">
            <a href="#" class="flex items-center px-4 py-2 text-red-600 hover:bg-red-50 rounded-lg transition text-sm font-semibold">
    ...
    <main class="flex-1 h-full overflow-y-auto flex flex-col relative w-full">
        ...
        <header class="flex items-center justify-between bg-white px-4 py-3 border-b border-gray-200 lg:hidden sticky top-0 z-30 shadow-sm">
            ...
            <!-- Tombol Hamburger -->
            <button onclick="toggleSidebar()" class="text-gray-500 hover:text-[#00236F] focus:outline-none bg-gray-50 hover:bg-gray-100 p-2 rounded-lg transition">
