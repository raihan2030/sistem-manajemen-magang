        ...
        <form id="skpdForm" action="{{ route('superadmin.kelola_akun.store') }}" method="POST">
            ...
            <div class="mb-8">
                ...
                <div class="mb-4">
                    ...
                    <select name="skpd_id" id="selectSkpd" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-[#00236F] focus:border-[#00236F] outline-none transition appearance-none bg-white text-gray-700 cursor-pointer" required>
                        <option value="" disabled selected>Pilih SKPD...</option>
                        @foreach($listSkpd as $skpd)
                            <option value="{{ $skpd->id }}" data-kode="{{ $skpd->kode_skpd }}" data-nama="{{ $skpd->nama_skpd }}">
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="email" name="email" id="inputEmail" placeholder="admin@skpd.banjarmasinkota.go.id" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-[#00236F] focus:border-[#00236F] outline-none transition" required>
                        <p class="text-[11px] text-gray-500 mt-1.5 leading-relaxed">Email ini akan digunakan untuk login sistem.</p>
                    </div>
                    
                    <!-- Kata Sandi -->
                    <div>
                        <label class="block text-sm font-bold text-[#1f2937] mb-2">Kata Sandi <span id="passReq" class="text-[#00236F]">*</span></label>
                        <div class="relative">
                            <input type="password" name="password" id="inputPassword" placeholder="••••••••" class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:ring-[#00236F] focus:border-[#00236F] outline-none transition tracking-widest">
                        ...
                        <p id="passHint" class="text-[11px] text-gray-500 mt-1.5 hidden">Kosongkan jika tidak ingin mengubah password.</p>
                    </div>
                </div>
            </div>
        ...
        <table class="w-full text-left border-collapse min-w-[700px]">
            ...
            <tbody id="tableBody" class="text-sm divide-y divide-gray-100">
                ...
                <tr id="row-{{ $item->id }}" class="hover:bg-gray-50/50 transition">
                    ...
                    <td class="px-6 py-5 align-middle text-center text-gray-800 tracking-widest font-bold row-password" data-raw="{{ $item->plain_password ?? 'Sandi Terenkripsi' }}">
                        ********
                    </td>
                    <td class="px-6 py-5 align-middle text-center">
                        <div class="flex items-center justify-center gap-3">
                            <!-- Tombol Edit -->
                            <button type="button" onclick="actionEdit('{{ $item->id }}', '{{ $item->skpd_id }}', '{{ $item->email }}')" 
                                class="w-8 h-8 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center hover:bg-amber-200 transition cursor-pointer" title="Edit Akun">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                            </button>

                            <!-- Tombol Hapus -->
                            <button type="button" onclick="confirmDelete('{{ $item->id }}', '{{ addslashes($item->skpd->nama_skpd ?? $item->name) }}')" 
                                class="w-8 h-8 rounded-full bg-red-100 text-red-600 flex items-center justify-center hover:bg-red-200 transition cursor-pointer" title="Hapus Akun">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>

                            <!-- Tombol Lihat Detail -->
                            <button type="button" onclick="actionView('{{ $item->skpd->kode_skpd ?? 'SKPD-' . $item->skpd_id }}', '{{ addslashes($item->skpd->nama_skpd ?? $item->name) }}', '{{ $item->email }}', '{{ $item->plain_password ?? 'Password terenkripsi (tidak dapat ditampilkan)' }}')" 
                                class="w-8 h-8 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center hover:bg-gray-200 transition cursor-pointer" title="Lihat Detail Kredensial">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                            </button>

                            <form id="delete-form-{{ $item->id }}" action="{{ route('superadmin.kelola_akun.destroy', $item->id) }}" method="POST" class="hidden">
                                @csrf
                                @method('DELETE')
                            </form>
                        </div>
...
<div id="detailModal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4">
    <div class="bg-white rounded-xl max-w-md w-full p-6 shadow-xl animate-fade-in">
        <div class="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
            ...
            <button onclick="closeModal()" class="text-gray-400 hover:text-gray-600 font-bold text-xl cursor-pointer">&times;</button>
        </div>
        <div class="space-y-3 text-sm mb-6">
            ...
            <div>
                <span class="block text-xs font-semibold text-gray-400 uppercase">Kata Sandi</span>
                <p id="modalPassword" class="font-mono font-bold text-red-600 bg-red-50 p-2 rounded border border-red-100 select-all"></p>
            </div>
        </div>
        <div class="flex justify-end">
            <button onclick="closeModal()" class="px-5 py-2 bg-[#00236F] text-white text-xs font-bold rounded-lg hover:bg-blue-900 transition cursor-pointer">
                Tutup
            </button>
        </div>
    </div>
...
<script>
    ...
    function actionView(kode, name, email, password) {
