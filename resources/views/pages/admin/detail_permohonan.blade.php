...
@section('content')
        ...
        <div class="lg:col-span-2 bg-white border border-gray-200 rounded-xl overflow-hidden shadow-xs flex flex-col">
            ...
            <div class="p-6 flex-grow flex flex-col justify-between gap-6">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-6">
                    <!-- NISN / NIM -->
                    <div>
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">NISN / NIM (Ketua)</span>
                    ...
                    <div>
                        <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1">Jurusan / Program Studi (Ketua)</span>
                ...
                <!-- Pembimbing Lapangan (Muncul jika sudah Diterima atau punya data) -->
                @if ($pengajuan->status === 'Diterima' || $pengajuan->nama_pembimbing)
                    <div class="pt-5 mt-1 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <span class="block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Pembimbing Lapangan</span>
                            @if ($pengajuan->nama_pembimbing)
                                <div class="flex items-center gap-2.5">
                                    <p class="text-sm font-bold text-[#1f2937]">{{ $pengajuan->nama_pembimbing }}</p>
                                    <a href="https://wa.me/{{ preg_replace('/^0/', '62', $pengajuan->no_wa_pembimbing) }}" target="_blank" class="text-[10px] font-bold text-emerald-700 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-200 flex items-center gap-1 hover:bg-emerald-100 transition cursor-pointer">
                                        <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01a1.05 1.05 0 00-.768.357c-.264.286-1.006.985-1.006 2.404s1.03 2.785 1.173 2.984c.143.198 2.03 3.102 4.922 4.352.691.298 1.23.477 1.65.61.693.22 1.324.189 1.821.114.558-.084 1.715-.7 1.956-1.376.241-.676.241-1.255.168-1.376-.073-.121-.272-.196-.57-.345z"/><path d="M12 2C6.477 2 2 6.477 2 12c0 1.763.456 3.42 1.258 4.861L2 22l5.312-1.218C8.715 21.542 10.315 22 12 22c5.523 0 10-4.477 10-10S17.523 2 12 2zm0 18.25c-1.48 0-2.921-.383-4.182-1.11l-.3-.178-3.111.712.727-3.036-.195-.311A8.204 8.204 0 013.75 12c0-4.551 3.7-8.25 8.25-8.25s8.25 3.699 8.25 8.25-3.7 8.25-8.25 8.25z"/></svg>
                                        {{ $pengajuan->no_wa_pembimbing }}
                                    </a>
                                </div>
                            @else
                        ...
                        <!-- Tombol Edit Pembimbing -->
                        <button type="button" onclick="editPembimbingLanjutan('{{ $pengajuan->nama_pembimbing }}', '{{ $pengajuan->no_wa_pembimbing }}')" class="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-white text-[#00236F] hover:bg-blue-50 border border-blue-200 text-[11px] font-bold rounded-lg transition cursor-pointer shadow-xs">
        ...
        <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            @if ($pengajuan->surat_balasan)
                <div class="border border-gray-200 rounded-xl p-4 flex items-center justify-between bg-white hover:border-blue-200 transition">
                    ...
                    <div class="flex items-center gap-3 text-xs font-bold text-[#00236F]">
                        <a href="{{ $pengajuan->surat_balasan_url }}" target="_blank" class="hover:underline flex items-center gap-1">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                            </svg>
    ...
    @if ($bisaDiverifikasi)
        ...
        <div class="bg-[#F8FAFC] border border-dashed border-gray-300 rounded-xl p-5 mb-10">
            ...
            <form id="formVerifikasi" action="{{ route('admin.permohonan.update', $pengajuan->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                ...
                <div class="mt-4 pt-4 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label for="namaPembimbing" class="block text-xs font-bold text-gray-700 mb-2">
                            Nama Pembimbing Lapangan <span class="text-red-600">(Wajib jika disetujui)</span>
                        </label>
                        <input type="text" name="nama_pembimbing" id="namaPembimbing" placeholder="Masukkan nama lengkap"
                            value="{{ old('nama_pembimbing', $pengajuan->nama_pembimbing ?? '') }}"
                            class="w-full bg-white border border-gray-200 rounded-xl px-3.5 py-3 text-xs text-gray-700 placeholder-gray-400 focus:ring-[#00236F] focus:border-[#00236F] outline-none transition">
                        @error('nama_pembimbing')
                            <p class="text-[11px] text-red-600 font-semibold mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        <label for="noWaPembimbing" class="block text-xs font-bold text-gray-700 mb-2">
                            No. WhatsApp Pembimbing <span class="text-red-600">(Wajib jika disetujui)</span>
                        </label>
                        <input type="text" name="no_wa_pembimbing" id="noWaPembimbing" placeholder="Contoh: 081234567890"
                            value="{{ old('no_wa_pembimbing', $pengajuan->no_wa_pembimbing ?? '') }}"
    ...
    <form id="formEditPembimbing" action="{{ route('admin.permohonan.update', $pengajuan->id) }}" method="POST" class="hidden">
        @csrf
        @method('PATCH')
    ...
    @if ($bisaDiverifikasi)
        function handleAction(statusTarget) {
            ...
            const namaPembimbingInput = document.getElementById('namaPembimbing');
            const noWaPembimbingInput = document.getElementById('noWaPembimbing');
            ...
            if (statusTarget === 'Diterima') {
                ...
                // 2. Validasi Pembimbing Lapangan
                const namaPembimbing = namaPembimbingInput ? namaPembimbingInput.value.trim() : '';
                const noWaPembimbing = noWaPembimbingInput ? noWaPembimbingInput.value.trim() : '';

                if (!namaPembimbing || !noWaPembimbing) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Identitas Pembimbing Belum Lengkap',
                        text: 'Harap isi Nama dan Nomor WhatsApp Pembimbing Lapangan untuk peserta magang!',
                        confirmButtonColor: '#00236F'
                    }).then(() => {
                        if (!namaPembimbing && namaPembimbingInput) namaPembimbingInput.focus();
                        else if (!noWaPembimbing && noWaPembimbingInput) noWaPembimbingInput.focus();
                    });
                    return;
                }
            }
