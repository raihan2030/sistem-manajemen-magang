...
{{-- DUMMY DATA PESERTA MAGANG DITERIMA DENGAN PENYESUAIAN NAMA KOLOM --}}
@php
    $stats = $stats ?? [
        'total_peserta' => 124,
        'berlangsung' => 86,
    ...
    $pesertas = $pesertas ?? [
        ...
        [
            ...
            'jurusan_prodi' => 'Administrasi Bisnis',
        ...
        [
            ...
            'institusi_asal' => 'Universitas Islam Kalimantan',
        ...
        [
            'id' => 6,
            'name' => 'Dina Rosita',
            'email' => 'dina.r@email.com',
            'institusi_asal' => 'Politeknik Negeri Banjarmasin',
            'jurusan_prodi' => 'Akuntansi',
...
<div class="bg-white border border-gray-200/90 rounded-2xl shadow-2xs overflow-hidden">
    ...
    <div class="p-5 border-b border-gray-100 flex flex-col lg:flex-row items-center justify-between gap-4">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <div class="relative w-full sm:w-80">
                ...
                <input type="text" id="searchInput" onkeyup="filterTable()" placeholder="Cari nama peserta atau instansi..." 
            ...
            <div class="w-full sm:w-44">
                <select id="statusFilter" onchange="filterTable()" class="w-full px-3.5 py-2.5 bg-gray-50/80 border border-gray-200 rounded-xl text-xs font-semibold text-gray-600 outline-none focus:border-[#00236F] transition cursor-pointer">
        ...
        <div class="flex items-center gap-3 w-full sm:w-auto justify-end">
            <a href="#" class="px-4 py-2.5 border border-emerald-600 text-emerald-700 bg-white hover:bg-emerald-50 rounded-xl text-xs font-bold transition shadow-2xs flex items-center gap-2">
            ...
            <a href="#" class="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-bold transition shadow-2xs flex items-center gap-2">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse min-w-[800px]">
            <thead>
                <tr class="bg-[#F4F7FF]/80 text-[11px] font-bold text-[#00236F] tracking-wider uppercase border-b border-gray-100">
            ...
            <tbody id="pesertaTableBody" class="text-xs divide-y divide-gray-100">
                ...
                @php
                    // Formatting Tanggal Mulai & Selesai untuk Periode Magang
                ...
                <tr class="peserta-row hover:bg-gray-50/60 transition" data-status="{{ $row['status'] }}">
                    ...
                    <td class="px-6 py-4.5 align-middle">
                        @if($row['status'] == 'Berlangsung')
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200/80 rounded-full text-[10px] font-bold">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                Berlangsung
                            </span>
                        @else
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-gray-100 text-gray-600 border border-gray-200 rounded-full text-[10px] font-bold">
                    ...
                    <td class="px-6 py-4.5 align-middle text-center">
                        <a href="{{ route('admin.permohonan.detail', ['id' => $row['id']]) }}" class="text-[#00236F] hover:text-blue-900 transition inline-block p-1" title="Lihat Detail Permohonan">
    ...
    <!-- 📍 PAGINASI DINAMIS BERFUNGSI -->
    <div class="px-6 py-4 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-gray-500">
        <div>
            Menampilkan <span id="text-range" class="font-semibold text-gray-700">0-0</span> dari <span id="text-total" class="font-semibold text-gray-700">0</span> data
        </div>

        <div class="flex items-center gap-1.5">
            <!-- Tombol Prev -->
            <button id="btnPrevPage" onclick="changePage(-1)" class="w-7 h-7 flex items-center justify-center rounded-lg border border-gray-200 text-gray-400 transition cursor-pointer">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
            </button>

            <!-- Container Angka Halaman (1, 2, 3, dst.) -->
            <div id="paginationNumbers" class="flex items-center gap-1.5"></div>

            <!-- Tombol Next -->
            <button id="btnNextPage" onclick="changePage(1)" class="w-7 h-7 flex items-center justify-center rounded-lg border border-gray-200 text-gray-700 transition cursor-pointer">
...
<script>
    let currentPage = 1;
    const itemsPerPage = 3; // Jumlah baris yang ditampilkan per halaman
    ...
    function renderTable() {
        ...
        // Tampilkan hanya baris sesuai paginasi aktif
        filteredRows.slice(startIdx, endIdx).forEach(row => {
            row.style.display = '';
        });

        // Update Teks Indikator Jumlah Data
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            btn.innerText = i;
            if (i === currentPage) {
                btn.className = "w-7 h-7 flex items-center justify-center rounded-lg bg-[#00236F] text-white font-bold shadow-2xs";
            } else {
                btn.className = "w-7 h-7 flex items-center justify-center rounded-lg border border-gray-200 hover:bg-gray-100 text-gray-700 font-semibold transition cursor-pointer";
                btn.onclick = () => {
                    currentPage = i;
                    renderTable();
                };
    ...
    function filterTable() {
        currentPage = 1; // Reset ke halaman 1 setiap pencarian/filter diubah
        renderTable();
    }
